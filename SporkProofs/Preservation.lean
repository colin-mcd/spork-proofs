...
namespace Step
  theorem preserve_exitsig_stack_cons
    ...
    (req : P[f]!.B[c₁]!.r = P[f]!.B[c₂]!.r) :
      CallStack.exitsig P (K ⬝ ⟨f, ρ₁, X₁, c₁⟩) =
      CallStack.exitsig P (K ⬝ ⟨f, ρ₂, X₂, c₂⟩)
    := by simp at req; cases K <;> simp[req]

  @[simp] theorem preserve_exitsig_stack_cons_cons {P K k k'} :
      (K ⬝ k ⬝ k').exitsig P = (K ⬝ k).exitsig P :=
    by cases K <;> rfl

  @[simp] theorem preserve_exitsig_stack_cons_nonnil {P K k} :
      K ≠ ∅ → (K ⬝ k).exitsig P = K.exitsig P := by
    cases K <;> intro nn
  ...
  @[simp] theorem preserve_exitsig_stack_append {P K k} : {K' : CallStack} ->
      CallStack.exitsig P (K ⬝ k ++ K') = CallStack.exitsig P (K ⬝ k)
    | .nil => rfl
    | K' ⬝ k' => preserve_exitsig_stack_cons_nonnil
                   (stack_append_left_nonnil (K ⬝ k) K' (by simp)) ▸
                 preserve_exitsig_stack_append (K' := K')

  theorem preserve_exitsig {P : Program} : {R R' : ThreadTree} ->
                         P ⊢ R ↦ R' -> P ⊢ R WF-tree ->
                         R.exitsig P = R'.exitsig P
  | R, R', s => by
    cases s <;> intro Rwf
    · case congr_parent Rp Rp' Rc s =>
        simp only [ThreadTree.exitsig]; match Rwf with
          | .node Rpwf Rcwf Rpp Rcp => exact preserve_exitsig s Rpwf
    · case congr_child Rp Rc Rc' s =>
        rfl
    · case stmt f K ρ X b e v c a =>
      apply preserve_exitsig_stack_cons
      rfl
    · case goto f K ρ X b bnext =>
      apply preserve_exitsig_stack_cons
      have flt : f < P.size := Rwf.get.K.current.flt
      have x := congrArg (·.2) Rwf.get.c.goto_bnext.bsig
      simp at x
      rw[getElem!_pos P f flt,
         getElem!_pos P[f].B bnext.b Rwf.get.c.goto_bnext.blt,
         getElem!_pos P[f].B b (by have y := Rwf.get.K.current.blt
                                   simp at y; rw[List.length_map]; exact y)]
      simp; exact x.symm
    · case ite_true f K ρ X b cond bthen belse n a neq0 =>
      apply preserve_exitsig_stack_cons
      have flt := Rwf.get.K.current.flt
      rw[getElem!_pos P f flt]
      rw[getElem!_pos P[f].B bthen.b Rwf.get.c.ite_bthen.blt]
      rw[getElem!_pos P[f].B b (by have y := Rwf.get.K.current.blt
                                   simp at y; rw[List.length_map]; exact y)]
    ...
    · case retn f g K ρ X Y y b bret =>
      apply preserve_exitsig_stack_cons (K := K)
      simp
    ...
    · case spoin_prom f g K ρ X b b' bunpr bprom Y y bspwn u =>
      apply preserve_exitsig_stack_cons
      have flt := Rwf.parent.get.K.current.flt
      rw[getElem!_pos P f flt]
      rw[getElem!_pos P[f].B bprom.b Rwf.parent.get.c.spoin_bprom.blt]
      rw[getElem!_pos P[f].B b (by have y := Rwf.parent.get.K.current.blt
                                   simp at y; rw[List.length_map]; exact y)]
  ...
  theorem preservation {P : Program} (Pwf : P WF-program) :
    ...
    · case goto f K ρ X b bnext wf =>
        ...
        · have h : P[f]!.B[bnext.b]! = _ := wf.c!.goto_bnext.bsig!
    ...
    · case call f g K ρ b X x bret wf =>
        ...
        · have p := wf.K.tail
          have s := congrArg BlockSig.r wf.c!.call_bret.bsig!
          simp only [StackFrame.ret!, getters, CallStack.bsig!, StackFrame.bsig!] at p s ⊢
          rw[s]
          exact p
        · apply goto_rets!h wf.K.current
          · have glt : g < P.size := by simpa using wf.c.call_flt
            have zlt : 0 < P[g].B.length := Pwf⁅g⁆.head.zero_lt_map
            have gsig : P[g].B[0].r = .retn P[g].fsig.ret :=
    ...
    · case retn f g K ρ X Y y b bret wf =>
        have ypf : (Y[y]'wf.c.retn_args).length = P[g]!.B[b]!.r.n :=
    ...
    · case spork f K ρ X b bbody bspwn wf =>
        ...
        · apply StackFrame.WF.mk
          · apply And.intro
            · have apf := congrArg BlockSig.Γ wf.c.spork_bbody.bsig
            ...
            · have bpf := congrArg BlockSig.σ wf.c.spork_bbody.bsig
        ...
        · have h : P[f]!.B[bbody.b]! = _ := wf.c!.spork_bbody.bsig!
          simpa only [StackFrame.ret!, getters, h] using wf.K.tail
    ...
    · case spoin_unpr f K bspwn ρ X b bunpr bprom wf =>
        ...
        · apply StackFrame.WF.mk
          · apply And.intro
            · have apf := congrArg BlockSig.Γ wf.c.spoin_bunpr.bsig
              simp only [getters] at apf
            ...
            · have bpf := congrArg BlockSig.σ wf.c.spoin_bunpr.bsig
              simp only [getters, CallStack.bsig, StackFrame.bsig,
                         SpawnDeque.pushsig, List.tail_cons] at bpf
              rw[bpf]
          · exact wf.K.current.ρwf.unpush
        · have h : P[f]!.B[bunpr.b]! = _ := wf.c!.spoin_bunpr.bsig!
          simpa only [StackFrame.ret!, getters, h] using wf.K.tail

    · case spoin_prom f g K ρ X b b' bunpr bprom Y y bspwn K_unpr_nil Rpwf Rcwf ret_prom_nil heq =>
        have flt : f < P.size := Rpwf.get.K.current.flt
        have glt : g < P.size := Rcwf.get.K.current.flt
        apply (· ⬝wf ·)
        · apply StackFrame.WF.mk
          · apply And.intro
            · have apf := congrArg BlockSig.Γ Rpwf.get.c.spoin_bprom.bsig
              ...
              simp only [getters, ThreadTree.promsig,
                         ...
                         ThreadTree.exitsig, Thread.exitsig, CallStack.exitsig] at heq
              ...
              have bpf := Rcwf.get.c.retn_length
              simp only [getters, CallStack.head, CallStack.bsig, StackFrame.bsig] at bpf
              rw[← bpf, ← heq]
              exact Nat.le_refl _
            · have bpf := congrArg BlockSig.σ Rpwf.get.c.spoin_bprom.bsig
              simp only [getters, CallStack.bsig, StackFrame.bsig,
                         SpawnDeque.pushsig, List.tail_cons] at bpf
              rw[bpf]
              simp[Rpwf.get.K.current.bsig.2]
          · have a : P[f].B; K.allPromoted ⊢ ⟨[], bspwn :: ρ⟩ WF-deque :=
              Rpwf.get.K.current.ρwf
            exact ⟨a.1, by simp[K_unpr_nil]; exact rfl⟩
        · have h : P[f]!.B[bprom.b]! = _ := Rpwf.get.c!.spoin_bprom.bsig!
