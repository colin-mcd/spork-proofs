...
namespace CallStack
  ...
  @[simp] def last : (K : CallStack) -> K ≠ nil -> StackFrame
    | nil ⬝ k, p => k
    | K  ⬝ k ⬝ _k', p => last (K ⬝ k) (by simp)

  @[simp] def last! : (K : CallStack) -> StackFrame
    | nil => default
    | nil ⬝ k => k
    | K  ⬝ k ⬝ _k' => last! (K ⬝ k)

  @[simp] theorem last_eq_last! : {K : CallStack} -> (nn : K ≠ nil) -> K.last nn = K.last!
    | nil ⬝ _k, _p => rfl
    | K ⬝ k ⬝ _k', _p => last_eq_last! (K := K ⬝ k) CallStack.noConfusion
